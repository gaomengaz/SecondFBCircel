...
- (void)setupToolbar {
    ...
    button.frame = CGRectMake(261, 7, 50, 30.0f);
    button.titleLabel.font = [UIFont systemFontOfSize:16.0];
    [button setTitle:NSLocalizedString(@"完成", @"完成") forState:UIControlStateNormal];
    [button setBackgroundImage:[UIImage imageNamed:@"wancheng88_52"] forState:UIControlStateNormal];
...
- (void)setupTextField {
    ...
    _mTextField.layer.cornerRadius = 2.0;
...
- (void)viewDidLoad {
    ...
    self.navigationItem.title = @"感谢您的反馈";
...
- (void)setBackgroundColor {
    ...
    if ([self.mToolBar respondsToSelector:@selector(setBackgroundImage:forToolbarPosition:barMetrics:)]) {
        UIImage *image = [self imageWithColor:RGBCOLOR(236, 236, 236)];
...
- (void)setBackButton {
    ...
    [backBtn setTitle:NSLocalizedString(@"关闭", @"关闭") forState:UIControlStateNormal];
...
- (IBAction)sendFeedback:(id)sender {
    if ([self.mTextField.text length]==0||[self.mTextField.text isEqualToString:@" "]||[self.mTextField.text isEqualToString:@" "]) {
        UIAlertView *alertv=[[UIAlertView alloc]initWithTitle:@"提示" message:@"输入内容为空，请重新输入" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
        [alertv show];
    }
