...
+(NSString *)getAuthkey{
    ...
    NSString *str_authkey=[NSString stringWithFormat:@"%@",[[NSUserDefaults standardUserDefaults]objectForKey:AUTHERKEY]];
    return str_authkey;
...
+(NSString *)getUsername{
    ...
    NSString *str_uid=[NSString stringWithFormat:@"%@",[[NSUserDefaults standardUserDefaults]objectForKey:USERNAME]];
    return str_uid;
...
+(NSString *)getUserFace{
    ...
    NSString *str_uid=[NSString stringWithFormat:@"%@",[[NSUserDefaults standardUserDefaults]objectForKey:USERFACE]];
    return str_uid;
...
+(NSArray *)exChangeFriendListByOrder:(NSMutableArray *)theArray
{
    ...
    {
        NSInteger sect = [theCollation sectionForObject:info collationStringSelector:@selector(zkingPaixufirstname)];//getLastName是实现中文安拼音检索的核心，见NameIndex类
    ...
    {
        NSArray *sortedSection = [theCollation sortedArrayFromArray:sectionArray collationStringSelector:@selector(zkingPaixufirstname)]; //同
