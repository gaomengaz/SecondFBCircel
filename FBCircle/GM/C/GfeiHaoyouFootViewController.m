//
//  GfeiHaoyouFootViewController.m
//  FBCircle
//
...
@interface GfeiHaoyouFootViewController ()

@end

@implementation GfeiHaoyouFootViewController
...
-(void)loadWenZhangBlockWithArray:(NSMutableArray*)array{
    ...
    if (_isupMore) {//是加载更多的话把请求到的文章加到原来的数组中
        ...
        if (self.wenzhangArray.count>=10) {
            [_upMoreView stopLoading:4];
        }
    ...
    int count = MIN(self.wenzhangArray.count, 10);
...
//    }
    ...
    //前十条信息的数组
    NSMutableArray *qianshiArray = [NSMutableArray arrayWithCapacity:1];
    for ( int i = 0; i<count; i++) {
        FBCircleModel *timeWZ = self.wenzhangArray[i];
        timeWZ.time = NO;
        timeWZ.timeStr = [GTimeSwitch testtime:timeWZ.fb_deteline];
        [qianshiArray addObject:timeWZ];
    }
    ...
    for (int i = 0; i<count; i++) {
        ...
        for (int j = i+1; j<count; j++) {
            FBCircleModel *wz1 = qianshiArray[i];
            FBCircleModel *wz2 = qianshiArray[j];
            //判断时间
